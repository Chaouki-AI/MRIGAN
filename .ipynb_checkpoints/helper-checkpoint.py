...
def load_dl(args): 
    images = glob.glob(f'{args.data_path_set1}/**/**/*.jpg')[:]
    dcms   = glob.glob(f'{args.data_path_set2}/**/**/*.dcm') if args.data_path_set2 is not None else None
    images = images + dcms if dcms is not None else images
    total = len(images) 
...
def evaluator(args, model, test_loader, epoch, device, writer):
    ...
    with torch.no_grad():
        ...
        writer.add_scalar('Metrics/ssim', values['ssim'], epoch + 1)
